...
BEGIN
  --- MODE's
  mode_state_machine : PROCESS (reset_n, clock, mode, ir_wire, run)
  BEGIN
    IF(reset_n = '0') THEN
      mode_state <= IDLE;
  ...
  --- STEPS's
  steps_counting : PROCESS (reset_n, prescaler, pwm_counter, pwm_state, run)
  BEGIN
    IF(reset_n = '0') THEN
      steps_counter <= 0;
      steps_output_wire <= 0;
	  ELSIF(run = '0') then
    	 steps_counter <= 0;
      steps_output_wire <= 0;
  	ELSIF(rising_edge(prescaler) AND pwm_counter = 0) THEN
			IF(direction = LEFT) THEN
