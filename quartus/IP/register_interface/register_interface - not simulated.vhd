...
entity register_interface is
  ...
  (
    ...
    modeBits_mcu  : out std_logic_vector(7 downto 0);   --! output of Mode bits for mcu
    speedReg_mcu  : out std_logic_vector(2 downto 0);   --! output of speedReg for mcu
    stepsReg_mcu  : in std_logic_vector(31 downto 0);   --! input for stepsReg for mcu
    irq_mcu       : in std_logic                        --! irq input for mcu
  );
...
begin
  ...
  if (reset_n = '0') then
    ...
    else if(CONV_INTEGER(irq_mcu) /= 0) then -- set IR-bit when irq_mcu is not '0'
      ctrlReg(7 downto 6) <= irq_mcu;
    else
      ctrlReg <= ctrlReg;
    end if;
  ...
  if (reset_n = '0') then
    ...
    else if(CONV_INTEGER(stepsReg_mcu) /= 0)) then  -- write stepsReg from mcu when mcu input is not '0'
      stepsReg <= stepsReg_mcu;
    else
      stepsReg <= stepsReg;
    end if;
  end if;
  ...
  -- extra mcu outputs
  modeBits_mcu <= ctrlReg(5 downto 2);
  speedReg_mcu <= speedReg;
